...
def show_player_score():
    ...
    for i in range(1, 7):
        ...
        if len(player_hand[i]) < min_cards[i]:
            valid_hand = False
            msg = "Pyramid Hand is missing at least one card - Try again"
            # print (message)
    ...
    if valid_hand == True:
        ...
        for i in range (1,6):
            ...
            if player_hand_score[i+1] <= player_hand_score[i]:
                valid_hand = False
                msg = "Pyramid Hands are in proper order - Try again"
                # print (message)

    if valid_hand == False:
        # msg = "Player Hand Invalid - Try Again"
...
client_socket.connect(("66.31.208.250", 55555))
...
r = tk.Canvas(results, height=7*Y_GAP+10+Y_OFFSET, width=10*X_GAP+X_OFFSET, bg="light blue", relief="raised")
count = 0
xloc1 = 11 * X_GAP + 10
yloc1 = 2 * Y_GAP + 10
xloc2 = xloc1 + 2 * X_GAP
